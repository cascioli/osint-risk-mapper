"""Tool schemas for all OSINT tools — Gemini FunctionDeclaration + OpenAI JSON Schema."""
...
# Raw tool specs: (name, description, properties_dict, required_list)
# Properties use JSON Schema format (lowercase types) — valid for both Gemini and OpenAI.
...
def get_tool_declarations() -> list[genai_types.FunctionDeclaration]:
    """Return Gemini FunctionDeclaration list for use with google-genai SDK."""
    return [
        ...
        for name, description, props, required in _TOOL_SPECS
    ]


def get_openai_tools() -> list[dict]:
    """Return OpenAI-compatible tools list (JSON Schema, lowercase types)."""
    tools = []
    for name, description, props, required in _TOOL_SPECS:
